...
def getXs():  # read poi vector via pre-training in word2vec tool
    fpointvec = open('data/bri_vector-250.txt', 'r')  # 
    #     table_X={}  #
    item = 0
...
def read_y():
    friendlist = open('data/bri_over_edges.txt', 'r')
...
def unkown_tra():
    ...
    funknown = open('data/bri_maybe_edges.txt', 'r')
...
def readtraindata():
    ...
    for i in range(len(tempT)):
        if (int(tempT[i][0]) in all_U_List): #INT TYPE
            ...
            TRA_ALL.append(int(tempT[i][0])) #store user ID
    ...
    temp_userY = list()
    temp_seqlens = list()
    User = 0  # record the number of users
    rate = 0.5
    for index in range(len(pointT)):
        if (userT[index] != flag or index == (len(pointT) - 1)):
            ...
            if (count > 1):  # 
                ...
                test_T += (pointT[int((index - math.ceil(count * rate))):index])  # test data
                test_UserT += (userT[int((index - math.ceil(count * rate))):index])  # test data
...
def train_tuf():
    ...
    with tf.Session() as sess:
        sess.run(initial)
        #saver.restore(sess, './temp/BK_LSTM.pkt')
        ...
        for epoch in range(train_iters): #train_iters
            ...
            while step < len(new_trainT)//batch_size:
                ...
                pres,train_cost, pred_out,train_optimizer=sess.run([pred,cost_class,pred_top,optimizer_class],feed_dict={encoder_embed_input:#
            ...
            saver.save(sess, './temp/BK_LSTM.pkt')
        # draw pic
        # print TOTAL_LOSS
        draw_pic(TOTAL_LOSS)
...
def save_metrics(LEARN_RATE,TRAIN_RECORED,RECOVERY_RECORED,RECOMMEND_RECORED):
    files=open('Result/metric_lstm.txt','a+')
...
def save_loss(TOTAL_LOSS):
    fopen=open('Result/BK_loss_lstm.txt','w')
...
def pad_sentence_batch(sentence_batch, pad_int):
    max_sentence = max([len(sentence) for sentence in sentence_batch]) #compute the max length
