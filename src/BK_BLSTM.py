...
n_classes=92
train_iters=100 #num for training
...
with tf.name_scope("w_b"):
   weights={
         'out':tf.Variable(tf.random_normal([2*n_hidden,2*n_classes]))
    }
   biases={
...
def getXs():  # read trajectory vectors
    fpointvec = open('data/bri_vector-250.txt', 'r')  # get check-in vector that already trained by word2vec
    #     table_X={}  #
    item = 0
