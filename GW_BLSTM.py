...
n_classes=201
train_iters=120 #nums for training
...
total_T = list()
total_U = list()
total_seqlens = list()  # original length
...
def train_tuf():
    ...
    with tf.Session() as sess:
        sess.run(initial)
        #saver.restore(sess, './temp/GW_BLSTM.pkt')
...
def save_metrics(LEARN_RATE,TRAIN_RECORED,RECOVERY_RECORED,RECOMMEND_RECORED):
    files=open('Result/GW_metric_blstm.txt','a+')
...
def save_loss(TOTAL_LOSS):
    fopen=open('Result/GW_loss_blstm.txt','w')
...
def acc_compute(pred,label): #统计其朋友圈的朋友准确率 都是掩码注意不死
    ...
    for step in range(len(pred)):
        ...
        for i in range(len(step_pred)):
            if(step_label[i]==1): #true label
                count+=1
            if (step_pred[i]==1): #logit label
                length+=1
